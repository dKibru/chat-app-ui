...
type ChatUser = {
  ...
  const chatHistory : ChatMsg[] = [
      { body: "start", sender_id: "4321", "seen_at" : "34/56/77" },
    { body: "a for", sender_id: "1234", "seen_at" : "34/56/77" },
    { body: "aaaa", sender_id: "4321", "seen_at" : "34/56/77" },
              ...
              if (f.sender_id == props.me){
                return (
                    <div className="clearfix">
                        <div className="bg-green-300 float-right w-3/4 mx-4 my-2 p-2 rounded-lg clearfix">{f.body}</div>
                    </div>
                )
              }else {
                return (
                    <div className="clearfix">
                        <div className="bg-gray-300 w-3/4 mx-4 my-2 p-2 rounded-lg">{f.body}</div>
                    </div>
                )
              }
    ...
    return (
      <div>
        ...
        <div className="fixed w-full flex justify-between bg-green-100" style={{bottom: '0px'}}>
          <textarea value={name} onChange={e => setName(e.target.value)} className="flex-grow m-2 py-2 px-4 mr-1 rounded-full border border-gray-300 bg-gray-200 resize-none" rows={1} placeholder="Message..." style={{outline: 'none'}} defaultValue={""} />
          <button onClick={send} className="m-2" style={{outline: 'none'}}>
